...
use serde::{
	de::{self, value::U32Deserializer, DeserializeSeed, EnumAccess, IntoDeserializer, SeqAccess, VariantAccess, Visitor},
	Deserialize,
};

use crate::{binary_format::read_vli, small_db_errors::DeserializeError};
		...
		V: Visitor<'de>,
	{
		...
		impl<'de, 'a, R: Read> SeqAccess<'de> for Access<'a, R> {
				...
				T: DeserializeSeed<'de>,
			{
				if self.len > 0 {
					self.len -= 1;
					let value = DeserializeSeed::deserialize(seed, Deserializer { input: &mut self.de.input })?;
					Ok(Some(value))
				} else {
					Ok(None)
				}
