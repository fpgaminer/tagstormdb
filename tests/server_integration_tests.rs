...
async fn test_api_access_controls() {
	...
	call_api(&client, &admin_token, &format!("images/{}", image2_hash), Method::POST, None, StatusCode::CREATED).await;

	test_api(&client, &admin_token, &user_token, "tags/testtag", Method::POST, None, "tags/testtag", Method::POST, None, "post/tags").await;
	test_api(&client, &admin_token, &user_token, "tags/testtag", Method::DELETE, None, "tags/testtag", Method::DELETE, None, "delete/tags").await;
	test_api(&client, &admin_token, &user_token, &format!("images/{}", image1_hash), Method::POST, None, &format!("images/{}", image1_hash), Method::POST, None, "post/images").await;
	test_api(&client, &admin_token, &user_token, &format!("images/{}", image1_hash), Method::DELETE, None, &format!("images/{}", image1_hash), Method::DELETE, None, "delete/images").await;
	test_api(&client, &admin_token, &user_token, "images/0/tags/0", Method::POST, None, "images/0/tags/0", Method::POST, None, "post/images/tags").await;
	test_api(&client, &admin_token, &user_token, "images/0/tags/0", Method::DELETE, None, "images/0/tags/0", Method::DELETE, None, "delete/images/tags").await;
	test_api(&client, &admin_token, &user_token, "images/0/attributes/source/land/false", Method::POST, None, "images/0/attributes/source/land/false", Method::POST, None, "post/images/attributes").await;
	test_api(&client, &admin_token, &user_token, "images/0/attributes/source/land", Method::DELETE, None, "images/0/attributes/source/land", Method::DELETE, None, "delete/images/attributes").await;
	test_api(&client, &admin_token, &user_token, "users/1", Method::GET, None, "users/1", Method::GET, None, "get/users/1").await;
	test_api(&client, &admin_token, &user_token, "users", Method::GET, None, "users", Method::GET, None, "get/users").await;
	test_api(&client, &admin_token, &user_token, "users/1/tokens", Method::GET, None, "users/1/tokens", Method::GET, None, "get/users/1/tokens").await;
	test_api(&client, &admin_token, &user_token, "users/1/scopes", Method::POST, Some(&json!({"new_scopes": "post/users/1/scopes"})), "users/1/scopes", Method::POST, Some(&json!({"new_scopes": "post/users/1/scopes"})), "post/users/1/scopes").await;
	test_api(&client, &admin_token, &user_token, "images/0/imgops", Method::POST, None, "images/0/imgops", Method::POST, None, "post/images/imgops").await; // TODO
	test_api(&client, &admin_token, &user_token, "images/0/predict/tags", Method::GET, None, "images/0/predict/tags", Method::GET, None, "get/images/predict/tags").await;
	test_api(&client, &admin_token, &user_token, "images/0/predict/caption?prompt=test", Method::GET, None, "images/0/predict/caption?prompt=test", Method::GET, None, "get/images/predict/caption").await;
	test_api(&client, &admin_token, &user_token, "task-queue", Method::POST, Some(&json!({"group": "test", "data": "{}", "status": "waiting"})), "task-queue", Method::POST, Some(&json!({"group": "test", "data": "{}", "status": "waiting"})), "post/task-queue").await;
	test_api(&client, &admin_token, &user_token, "task-queue/1", Method::DELETE, None, "task-queue/1", Method::DELETE, None, "delete/task-queue").await;
	test_api(&client, &admin_token, &user_token, "task-queue", Method::GET, None, "task-queue", Method::GET, None, "get/task-queue").await;
	test_api(&client, &admin_token, &user_token, "task-queue", Method::POST, Some(&json!({"group": "test", "data": "{}", "status": "waiting"})), "task-queue", Method::POST, Some(&json!({"group": "test", "data": "{}", "status": "waiting"})), "post/task-queue").await;
	test_api(&client, &admin_token, &user_token, "task-queue/test/acquire", Method::POST, None, "task-queue/test/acquire", Method::POST, None, "post/task-queue/acquire").await;
	test_api(&client, &admin_token, &user_token, "task-queue/1/finish", Method::POST, None, "task-queue/1/finish", Method::POST, None, "post/task-queue/acquire").await;
	...
	test_api(&client, &admin_token, &user_token, "users/1/login_key", Method::POST, Some(&json!({"new_login_key": hex::encode(&user_login_key2)})), "users/1/login_key", Method::POST, Some(&json!({"new_login_key": hex::encode(&user_login_key2)})), "post/users/1/login_key").await;
	...
	test_api(&client, &admin_token, &user_token2, "users/me/tokens/invalidate", Method::POST, Some(&json!({"token": &user_token})), "users/me/tokens/invalidate", Method::POST, Some(&json!({"token": &user_token2})), "delete/users/1/tokens").await;
	...
	call_api(&client, &user_token, "search/images?query=id=0&select=id", Method::GET, None, StatusCode::UNAUTHORIZED).await;
...
async fn call_api(client: &Client, admin_token: &str, api: &str, method: Method, body: Option<&serde_json::Value>, expected_status: StatusCode) {
	let request = client
		.request(method, format!("http://127.0.0.1:7734/{}", api))
	...
	assert_eq!(response.status(), expected_status, "Expected {} to return status {}, {}", api, expected_status, response.text().await.unwrap());
}

async fn test_api(client: &Client, admin_token: &str, user_token: &str, positive_api: &str, positive_method: Method, positive_body: Option<&serde_json::Value>, negative_api: &str, negative_method: Method, negative_body: Option<&serde_json::Value>, scope: &str) {
	// Set user's scopes
	...
	let request = if let Some(body) = positive_body {
		request.json(body)
	} else {
		request
	};
	...
	assert!(response.status().is_success(), "Expected {} to succeed: {}", positive_api, response.text().await.unwrap());
	...
	let request = client
		...
		.bearer_auth(user_token);
	let request = if let Some(body) = negative_body {
		request.json(body)
	} else {
		request
	};
...
async fn test_upload(client: &Client, admin_token: &str, user_token: &str, data: &[u8]) {
	...
	let response = client.post(format!("http://127.0.0.1:7734/upload_image")).multipart(form).bearer_auth(user_token).send().await.unwrap();
...
async fn set_user_scopes(client: &Client, user_token: &str, scopes: &str) {
	...
	assert!(
		...
		"Failed to set user scopes, status: {}",
		response.status()
	);
}
...
async fn create_server() -> (tokio::process::Child, LoginKey, LoginKey, tempfile::TempDir, ServerHandle) {
	...
	std::fs::write(&secrets_path, &serde_json::to_vec(&json!({
		"server_secret": hex::encode(server_secret),
	})).unwrap()).expect("Failed to write server secrets");
	...
	let (admin_login_key, user_login_key) = {
		...
		database.add_user("admin".to_string(), admin_hashed_login_key, admin_scopes.to_string()).await.expect("Failed to add admin user");
		...
		database.add_user("user".to_string(), user_hashed_login_key, "post/users/1/tokens".to_string()).await.expect("Failed to add user");
	...
	let prediction_server = HttpServer::new(move || {
		actix_web::App::new()
			.service(actix_web::web::resource("/predict").to(|| async {
				actix_web::HttpResponse::Ok().json(vec!["tag1", "tag2", "tag3"])
			}))
			.service(actix_web::web::resource("/tag_assoc").to(|| async {
				actix_web::HttpResponse::Ok().json(vec!["tag1", "tag2", "tag3"])
			}))
			.service(actix_web::web::resource("/caption").to(|| async {
				actix_web::HttpResponse::Ok().json("A test caption")
			}))
	})
	.workers(1)
	.bind(("127.0.0.1", 9184))
